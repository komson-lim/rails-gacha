class Banner < ApplicationRecord
    ...
    def getItem()
        Item.joins(:banner_items).where("banner_items.banner_id = #{self.id}").select("items.id, items.name, items.rarity, banner_items.rate, items.url").order("items.rarity DESC")
    end
    def getRate()
